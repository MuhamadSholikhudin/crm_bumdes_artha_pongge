...
if (isset($_POST['simpanberkas_pemasangan'])) {
    ...
    if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
        if ($ukuran < 1044070) {
            ...
            $upload_guru = move_uploaded_file(
                $file_tmp,
                $lokasi_foto . '/foto_berkas/' . $nama_file
            );
            if ($upload_guru) {
    ...
    if (isset($_FILES['foto_berkas'])) {
        ...
        if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
            if ($ukuran < 1044070) {
                $nama_file = $YMDhis . $_FILES['foto_berkas']['name'];
                unlink(
                    $lokasi_foto . '/foto_berkas/' . $_POST['foto_berkas_old']
                );
                $upload_guru = move_uploaded_file(
                    $file_tmp,
                    $lokasi_foto . '/foto_berkas/' . $nama_file
    ...
    $process = UpdateOneData(
        'berkas_pemasangan',
        $data,
        ' WHERE id_berkas_pemasangan ="' . $_POST['id_berkas_pemasangan'] . '"'
    );
    $_SESSION['message'] = 'Data Berkas Pemasangan ' . $process['message'];
    header('Location: ' . $url . '/app/berkas_pemasangan/index.php');
    exit();
    ...
    $process = DeleteOneData(
        'berkas_pemasangan',
        'WHERE id_berkas_pemasangan = "' . $_GET['id_berkas_pemasangan'] . '"'
    );
    $_SESSION['message'] = 'Data Berkas Pemasangan ' . $process['message'];
    header('Location: ' . $url . '/app/berkas_pemasangan/index.php');
    exit();
