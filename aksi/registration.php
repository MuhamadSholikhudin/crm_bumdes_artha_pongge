...
if (isset($_POST)) {
    if ($_POST['password'] != $_POST['password_confirm']) {
        $_SESSION['unvalid_username'] =
            'Data pelanggan ' .
            $_POST['username'] .
            ' gagal di buat coba check form passwod yang di isi !';
        header('Location: ' . $url . '/app/registration.php');
        exit();
    }
    $username_check = QueryOnedata(
    ...
    if ($login->num_rows > 0) {
        ...
        $id_p = 1;
        $terahir_pelanggan = QueryOnedata(
            'SELECT * FROM pelanggan ORDER BY id_pelanggan DESC '
        );
        if ($terahir_pelanggan->num_rows > 0) {
            $id_p =
                Rplc('PL', $terahir_pelanggan->fetch_assoc()['id_pelanggan']) +
                $id_p;
        }
        ...
        $_SESSION['message'] =
            'Data pelanggan ' . $_POST['username'] . ' berhasil di buat !';
